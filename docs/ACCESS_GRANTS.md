...
`@pegma/authorization-tokens`. The host still owns signing keys, the HTTPS
endpoint that publishes the package's public JWKS projection, and the
`@pegma/storage-core` `Store` used for atomic replay consumption.
...
- `jti` is a unique, unpredictable 256-bit value encoded as exactly 43
  ...
  atomically reserved before signing in the host-supplied Store under the exact
  `(iss, application_id, jti)` tuple. Reservations are retained indefinitely,
  so it is never reused under the same issuer and application across processes
  sharing that Store. It participates in
...
The executable contract and runtime tests exercise these semantics against the
storage-core reference store. They do not prove a production backend's
multi-process atomicity, durability, retention, or clock quality. The runtime
declares `authorization_access_grant_replays` against
`@pegma/storage-core` and uses a host-supplied `Store`; it adds no bespoke
persistence here. Its records may be retained indefinitely. Issuance
separately declares `authorization_access_grant_jti_reservations` and requires
atomic insert-if-absent before signing. A reservation outage, ambiguous write,
conflict, or corrupt result fails closed, and reservations are never deleted.
